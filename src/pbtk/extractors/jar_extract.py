    ...
    with JarWrapper(path) as jar:
        ...
        for i, cls in enumerate(jar.classes):
            ...
            if has_out_constructor and (
                (
                    (has_out_relevant_string or has_out_relevant_string_old)
                    and is_legit_out_class
                )
                or has_out_relevant_string_nano
                ...
                has_constructor
                and is_legit_class
                and (calls_arraydecoder or has_relevant_string)
            ) or (
                has_constructor_nano
    ...
    for start, (call, end) in code.method_calls.items():
        ...
        if call_obj in [codedinputstream, *map_entry_cls] and (
            ...
            if not in_switch:
                ...
                if (
                    label_to_val and start < label_to_val[0][0]
                ):  # We're before the current switch case...
                    continue
                while (
                    label_to_val and start > label_to_val[0][1][1]
                ):  # We're after the next one in method...
    ...
    for start, (call, _) in code.method_calls.items():
        ...
        if call_obj in [
            ...
            *map_entry_cls,
            ...
            if (
                call_obj not in map_entry_cls or fenumormsg
            ):  # Non map-type: store back information
