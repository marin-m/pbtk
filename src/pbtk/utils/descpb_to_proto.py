...
def parse_msg(desc, scopes, syntax):
    ...
    for enum in desc.enum_type:
        ...
        for val in enum.value:
            out2 += '%s = %s;\n' % (
                val.name,
                fmt_value(val.number, val.options),
            )
    ...
    if is_msg and desc.options.map_entry:
        return ' map<%s>' % ', '.join(
            min_name(i.type_name, scopes) if i.type_name else types[i.type]
            for i in desc.field
        )
...
types = {
    v: k.split('_')[1].lower() for k, v in FieldDescriptorProto.Type.items()
}
labels = {
    v: k.split('_')[1].lower() for k, v in FieldDescriptorProto.Label.items()
}
...
def fmt_field(field, scopes, blocks, syntax, extend=False):
    ...
    if out.startswith('map<'):
        line = out + ' %s = %s;\n' % (
            field.name,
            fmt_value(field.number, field.options, optarr=default),
        )
        out = ''
...
def min_name(name, scopes):
    ...
    while name and (
        ...
        or any(
            list_rfind(scope.split('.'), short_name[0]) > len(name)
            for scope in scopes
        )
    ):
...
def wrap_block(type_, value, desc=None, name=None):
    ...
    if desc:
        for option, optval in desc.options.ListFields():
            value = (
                'option %s = %s;\n'
                % (option.name, fmt_value(optval, desc=option))
                + value
            )
    ...
    if type_:
        out += '\n'.join(
            INDENT + line for line in value.strip('\n').split('\n')
        )
        out += '\n}\n\n'
    else:
        out += value
