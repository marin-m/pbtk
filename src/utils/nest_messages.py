    ...
    while still_to_merge:
        ...
        for msg, in_pkg in OrderedDict(mergeable).items():
            ...
            for field, referrer in sorted(
                ...
                if (
                    msg in msg_to_imports[top_referrer]
                    and top_referrer in msg_to_imports[msg]
                    and msg_to_topmost.get(referrer, referrer) != msg
                ) or duplicate_enumfields:
                    merge_and_rename(
                        msg,
                        referrer,
                        get_pkg(msg),
        ...
        if msg not in msg_to_topmost:
            ...
            for imported in msg_to_imports[msg]:
                ...
                if (
                    ...
                    and import_path not in file_obj.dependency
                ):
...
def merge_and_rename(
    ...
    other_names = [
        i.name
        for i in [
            *filter(lambda x: x.type != x.TYPE_GROUP, referrer_obj.field),
    ...
    while (
        new_name in other_names
        or (is_group and new_name.lower() in other_names)
        or (
            msg_pkg + new_name in msg_to_imports[top_path]
