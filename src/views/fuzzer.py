...
class QwordSpinBox(QAbstractSpinBox):
    ...
    def __init__(self, min_, max_, float_=False):
        ...
        self.editingFinished.connect(
            lambda: self.setValue(self.value(), update=False) or True
        )
...
class ProtobufItem(QTreeWidgetItem):
    def __init__(self, item, ds, app, path):
        ...
        if (
            self.required
            and not self.parent()
            and not self.app.pb_request.HasField(self.ds.name)
        ):
    ...
    def do_rename(self, new_name):
        ...
        for file_ in file_set:
            field_path = self.find_path_for_field(
        ...
        for i, msg in enumerate(msgs):
            if self.full_name.startswith(cur_name + msg.name + '.'):
                ...
                return self.find_path_for_field(
                    msg.nested_type, [*path, i, 3], cur_name + msg.name
                )
